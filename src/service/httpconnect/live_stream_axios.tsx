...
import acitonLiveDto from "../dto/action-live-dto";
...
//
export const getActionInfo = async (callback: (a: acitonLiveDto) => void) => {
  // try {
  // 깃허브 api 중 유저의 정보를 받아오는 api를 사용
  const res: any = await axios({
    url: host + "board/279",
    ...
    .then(async function (response) {
      ...
      // const result: JSON = await response.data;
      const result: acitonLiveDto = await response.data.result;

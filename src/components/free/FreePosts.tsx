...
export default function FreePosts() {
  ...
  const getItems = useCallback(async () => {
    ...
    try {
      const response = await axios.get(
        `https://reptimate.store/api/board?page=${page}&size=20&order=DESC&category=free`
      );
      setData(
        (prevData) =>
          ({
  ...
  if (data !== null && data.result.items) {
    ...
    return (
      <section>
        <ul className="mt-10 grid grid-cols-2 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4">
          {itemlist.map((post) => (
            <li key={post.idx}>
              <PostCard post={post} />
            </li>

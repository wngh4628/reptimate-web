...
export default function ProgressBar(props:any) {
  ...
  return (
      <div className="flex flex-col items-center w-full ">
        <span className="text-base font-medium dark:text-white">{key}</span>
        <div className="flex justify-center items-center w-full">                
          ...
          <div className="flex justify-center items-center relative ml-12">
            <span className="absolute text-sm font-medium dark:text-white right-3">{unit !== '' ? value : (value === 50 ? '유' : '무')}</span>  
            <span className="absolute text-sm font-medium dark:text-white right-0">{unit}</span>
          </div>
